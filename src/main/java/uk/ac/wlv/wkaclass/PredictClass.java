...
public class PredictClass {
   ...
   /**
    * 函数返回一个字符串，表示未分类Arff文件中的每个实例的预测类别。
    * @param arffTrainFile 训练数据集使用Arff格式的文件（arffTrainFile）。
    * @param sClassifier 使用指定的分类器（sClassifier）预测未分类的Arff文件（sUnclassifiedArff）中的类别，并返回预测结果。
    * @param sUnclassifiedArff 使用指定的分类器（sClassifier）预测未分类的Arff文件（sUnclassifiedArff）中的类别，并返回预测结果。
    * @param iClassFor0 参数iClassFor0指定哪个类别被视为“0”类，其他类别被视为“1”类。
    * @return
    */
